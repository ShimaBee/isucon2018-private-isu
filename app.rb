...
module Isuconp
  class App < Sinatra::Base
    ...
    helpers do
      ...
      def make_posts(results, all_comments: false)
        ...
        results.to_a.each do |post|
          post[:comment_count] = db.prepare('SELECT COUNT(*) AS `count` FROM `comments` WHERE `post_id` = ? limit 5').execute(
            post[:id]
          ).first[:count]
    ...
    get '/' do
      ...
      results = db.query('SELECT `id`, `user_id`, `body`, `created_at`, `mime` FROM `posts` ORDER BY `created_at` DESC limit 5')
    ...
    get '/@:account_name' do
      ...
      results = db.prepare('SELECT `id`, `user_id`, `body`, `mime`, `created_at` FROM `posts` WHERE `user_id` = ? ORDER BY `created_at` DESC limit 5').execute(
        user[:id]
      )
      posts = make_posts(results)
